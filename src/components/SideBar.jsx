export default function SideBar({ onMenuSelection, selection }) {
	const menuItems = ['Income', 'Expenses', 'Budget', 'Savings'];

	let menuItemHoverStyles = 'hover:font-medium hover:text-slate-800';

	return (
		<div className="flex h-full w-1/5 flex-col justify-between rounded-e-3xl bg-blue-300 text-center font-display">
			<h1
				className="mb-auto mt-6 cursor-pointer px-4 text-2xl font-medium text-slate-700 hover:text-slate-800"
				onClick={() => onMenuSelection(undefined)}
			>
				Ultimate Budget
			</h1>
